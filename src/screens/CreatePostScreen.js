...
export default function CreatePostScreen({ navigation }) {
  ...
  const handleSelectImage = async () => {
    try {
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        base64: true,           // Trả về chuỗi base64 của ảnh
        quality: 0.5,           // Chất lượng ảnh (0 đến 1)
      });
      if (!result.cancelled) {
  ...
  const handlePost = async () => {
    ...
    // Lấy chuỗi base64 nếu có ảnh, nếu không mảng rỗng
    const imageBase64s = selectedImage ? [selectedImage.base64] : [];

    const postData = {
      title: postText.length > 30 ? postText.substring(0, 30) + '...' : postText,
      content: postContent,
      imageBase64s,  // Mảng chứa ảnh (dạng base64)
    };
